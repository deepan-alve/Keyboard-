...
import com.azhagi.azhagikeys.ime.keyboard.CachedLayout
import com.azhagi.azhagikeys.ime.keyboard.DebugLayoutComputationResult
...
import org.azhagi.lib.snygg.SnyggMissingSchemaException
import java.text.SimpleDateFormat
import java.util.*

private val CardBackground = Color.Black.copy(0.6f)
private val DateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH-mm-ss", AzhagiLocale.default().base)
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for AzhagiKeys v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.
