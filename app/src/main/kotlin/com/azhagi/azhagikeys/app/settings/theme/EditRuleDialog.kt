...
import org.azhagi.lib.snygg.ui.NonNullSaver
...
internal fun EditRuleDialog(
    ...
    val possibleRuleTemplates = remember {
        buildList {
            ...
            AzhagiImeUi.elementNames.forEach { name ->
        ...
        Column {
            ...
            (currentRule as? SnyggElementRule)?.apply {
                ...
                DialogProperty(text = stringRes(R.string.settings__theme_editor__rule_selectors)) {
                    Row(
                        modifier = Modifier.AzhagiHorizontalScroll(),
                ...
                val codes = remember(currentRule) {
                    attributes[AzhagiImeUi.Attr.Code] ?: emptyList()
                }
                ...
                if (initCodeValue != null) {
                    EditCodeValueDialog(
                        ...
                        onAdd = {
                            currentRule = copy(
                                attributes = attributes.including(AzhagiImeUi.Attr.Code to it)
                            )
                        },
                        onDelete = {
                            currentRule = copy(
                                attributes = attributes.excluding(AzhagiImeUi.Attr.Code to it)
                            )
                        },
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = AzhagiImeUi.Attr.Mode,
                ...
                EnumLikeAttributeBox(
                    ...
                    attribute = AzhagiImeUi.Attr.ShiftState,
                    attributes = attributes,
                    setAttributes = { currentRule = copy(attributes = it) },
                    level = level,
                )
...
private fun EditCodeValueDialog(
    ...
    fun requestStartRecording() {
        ...
        if (!isAzhagiKeysEnabled || !isAzhagiKeysSelected) {
            lastRecordingToast?.cancel()
            lastRecordingToast = context.showShortToast(
                R.string.settings__theme_editor__code_recording_requires_default_ime_Azhagi,
                "app_name" to context.stringRes(R.string.azhagi_app_name),
            )
            ...
            AnimatedVisibility(visible = showKeyCodesHelp) {
                Column(modifier = Modifier.padding(bottom = 16.dp)) {
                    ...
                    AzhagiHyperlinkText(
                        ...
                        url = stringRes(R.string.AzhagiKeys__character_key_codes_url),
                    )
                    AzhagiHyperlinkText(
                        text = "Internal (github.com)",
                        url = stringRes(R.string.AzhagiKeys__internal_key_codes_url),
                    )
                }
            }
