...
object ZipUtils {
    fun readFileFromArchive(context: Context, zipRef: AzhagiRef, relPath: String) = runCatching<String> {
        when {
            zipRef.isAssets -> {
    ...
    fun zip(context: Context, srcRef: AzhagiRef, dstRef: AzhagiRef) =
        zip(context, FsDir(srcRef.absolutePath(context)), dstRef)

    fun zip(context: Context, srcDir: FsDir, dstRef: AzhagiRef) = runCatching {
    ...
    fun unzip(context: Context, srcRef: AzhagiRef, dstRef: AzhagiRef) =
        unzip(context, srcRef, FsDir(dstRef.absolutePath(context)))

    fun unzip(context: Context, srcRef: AzhagiRef, dstDir: FsFile) = runCatching {
