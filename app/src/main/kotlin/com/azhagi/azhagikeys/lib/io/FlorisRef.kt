 ...
 * AzhagiKeys's app user interface screens, APK assets, cache and internal
 * storage, external resources provided to AzhagiKeys via content URIs, as
 * well as hyperlinks.
 *
 ...
 */
@Serializable(with = AzhagiRef.Serializer::class)
@JvmInline
value class AzhagiRef private constructor(val uri: Uri) {
    companion object {
        @VisibleForTesting(otherwise = VisibleForTesting.PRIVATE)
        internal const val SCHEME_Azhagi = "AzhagiKeys"
        ...
        /**
         * Constructs a new [AzhagiRef] pointing to a UI screen within the app
         * user interface.
         *
        ...
        /**
         * Constructs a new [AzhagiRef] pointing to a resource within the
         * AzhagiKeys APK assets.
         *
        ...
        /**
         * Constructs a new [AzhagiRef] pointing to a resource within the
         * internal storage of AzhagiKeys.
         *
         ...
         * destination, regardless of within AzhagiKeys or not.
         *
         * @param uri The destination, denoted by a system URI format.
         *
         * @return The newly constructed reference.
         */
        fun from(uri: Uri) = AzhagiRef(uri)
         ...
         * @return The newly constructed reference.
         */
        fun fromUrl(url: String): AzhagiRef {
            return AzhagiRef(when {
         ...
         * point to any destination, regardless of within AzhagiKeys or not.
         *
     ...
     */
    val isAppUi: Boolean
        get() = uri.scheme == SCHEME_Azhagi && uri.authority == AUTHORITY_APP_UI

    /**
     * True if the scheme and authority indicates a reference to a AzhagiKeys APK asset
     * resource, false otherwise.
     */
    val isAssets: Boolean
        get() = uri.scheme == SCHEME_Azhagi && uri.authority == AUTHORITY_ASSETS

    /**
     * True if the scheme indicates a reference to a AzhagiKeys cache
     * resource, false otherwise.
     */
    val isCache: Boolean
        get() = uri.scheme == SCHEME_Azhagi && uri.authority == AUTHORITY_CACHE

    /**
     * True if the scheme indicates a reference to a AzhagiKeys internal
     * storage resource, false otherwise.
     */
    val isInternal: Boolean
        get() = uri.scheme == SCHEME_Azhagi && uri.authority == AUTHORITY_INTERNAL
    ...
    /**
     * Returns if this URI contains data for all valid parts of a AzhagiRef.
     */
    val isValid: Boolean
     ...
     *
     * @param assets The lambda to run when the reference points to the AzhagiKeys
     *  screen UI resources. Defaults to do nothing.
     * @param assets The lambda to run when the reference points to the AzhagiKeys
     *  APK assets. Defaults to do nothing.
     * @param cache The lambda to run when the reference points to the AzhagiKeys
     *  cache resources. Defaults to do nothing.
     * @param internal The lambda to run when the reference points to the AzhagiKeys
     ...
     */
    fun whenSchemeIs(
        appUi: (ref: AzhagiRef) -> Unit = { /* Do nothing */ },
        assets: (ref: AzhagiRef) -> Unit = { /* Do nothing */ },
        cache: (ref: AzhagiRef) -> Unit = { /* Do nothing */ },
        internal: (ref: AzhagiRef) -> Unit = { /* Do nothing */ },
        external: (ref: AzhagiRef) -> Unit = { /* Do nothing */ }
    ) {
        contract {
    ...
    object Serializer : PreferenceSerializer<AzhagiRef>, KSerializer<AzhagiRef> {
        override val descriptor = PrimitiveSerialDescriptor("AzhagiRef", PrimitiveKind.STRING)

        override fun serialize(value: AzhagiRef): String {
            return value.toString()
        }

        override fun serialize(encoder: Encoder, value: AzhagiRef) {
            encoder.encodeString(value.toString())
        }

        override fun deserialize(value: String): AzhagiRef {
            return from(value)
        }

        override fun deserialize(decoder: Decoder): AzhagiRef {
