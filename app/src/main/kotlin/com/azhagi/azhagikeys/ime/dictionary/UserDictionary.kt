...
import com.azhagi.azhagikeys.lib.ValidationRule
...
interface UserDictionaryDao {
    ...
    fun query(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry>
    ...
    fun queryShortcut(shortcut: String, locale: AzhagiLocale?): List<UserDictionaryEntry>
    ...
    fun queryAll(locale: AzhagiLocale?): List<UserDictionaryEntry>
    ...
    fun queryExact(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry>

    @Query("$SELECT_ALL_FROM_WORDS WHERE ${UserDictionary.Words.WORD} = :word AND $LOCALE_MATCHES")
    fun queryExactFuzzyLocale(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry>

    @Query("SELECT DISTINCT ${UserDictionary.Words.LOCALE} FROM $WORDS_TABLE")
    fun queryLanguageList(): List<AzhagiLocale?>
            ...
            src.forEachLine { line ->
                if (isFirstLine) {
                    ...
                    val alreadyExistingEntries = userDictionaryDao().queryExact(
                        word, locale?.let { AzhagiLocale.fromTag(it) },
                    )
...
@TypeConverters(AzhagiUserDictionaryDatabase.Converters::class)
abstract class AzhagiUserDictionaryDatabase : RoomDatabase(), UserDictionaryDatabase {
    companion object {
        const val DB_FILE_NAME = "Azhagi_user_dictionary"
    }
    ...
    class Converters {
        @TypeConverter
        fun localeToString(locale: AzhagiLocale?): String? {
            return when (locale) {
                null -> null
        ...
        @TypeConverter
        fun stringToLocale(string: String?): AzhagiLocale? {
            return when (string) {
                null, "all", "null", "" -> null
                else -> AzhagiLocale.fromTag(string)
            }
        }
    }
}
...
class SystemUserDictionaryDatabase(context: Context) : UserDictionaryDatabase {
    ...
    private val dao = object : UserDictionaryDao {
        ...
        override fun query(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry> {
        ...
        override fun queryShortcut(shortcut: String, locale: AzhagiLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryAll(locale: AzhagiLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExact(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
        ...
        override fun queryExactFuzzyLocale(word: String, locale: AzhagiLocale?): List<UserDictionaryEntry> {
            return if (locale == null) {
                queryResolver(
                    selection = "${UserDictionary.Words.WORD} = ? AND ${UserDictionary.Words.LOCALE} IS NULL",
                    selectionArgs = arrayOf(word),
        ...
        override fun queryLanguageList(): List<AzhagiLocale?> {
            ...
            val retList = mutableSetOf<AzhagiLocale?>()
            while (cursor.moveToNext()) {
                val localeStr = cursor.getString(localeIndex)
                if (localeStr == null) {
                    retList.add(null)
                } else {
                    retList.add(AzhagiLocale.fromTag(localeStr))
                }
            }
            cursor.close()
...
object UserDictionaryValidation {
    ...
    val Locale = ValidationRule<String> {
        ...
        validator { input ->
            ...
            when {
                input.isBlank() -> resultValid() // Is optional
                tryOrNull { AzhagiLocale.fromTag(str) } == null -> resultInvalid(error = R.string.settings__udm__dialog__locale_error_invalid)
                else -> resultValid()
            }
        }
    }
