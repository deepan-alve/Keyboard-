...
open class BreakIteratorGroup {
    private val charInstances = Cache.Builder().build<AzhagiLocale, GuardedByLock<BreakIterator>>()

    private val wordInstances = Cache.Builder().build<AzhagiLocale, GuardedByLock<BreakIterator>>()

    private val sentenceInstances = Cache.Builder().build<AzhagiLocale, GuardedByLock<BreakIterator>>()

    suspend fun <R> character(locale: AzhagiLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> word(locale: AzhagiLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun <R> sentence(locale: AzhagiLocale, action: (BreakIterator) -> R): R {
    ...
    suspend fun measureUChars(
        text: String,
        numUnicodeChars: Int,
        locale: AzhagiLocale = AzhagiLocale.default(),
    ): Int {
